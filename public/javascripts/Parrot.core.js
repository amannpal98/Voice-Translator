(function () {
    var speechSynthesisSupported = 'speechSynthesis' in window,
        ...
        spokenContent = document.getElementById('spokenContent'),
    ...
    eParrot.prototype.gTranslator = function (t) {
        // Display spoken text
        if (spokenContent) {
            spokenContent.textContent = t;
        }
        // Expand box when translation starts
        if (typeof expandDisplayArea === 'function') expandDisplayArea(true);
        this.request("http://localhost:3000/g-translator/result?q=" + encodeURIComponent(t) + "&tl=" + encodeURIComponent(voiceSelect.options[voiceSelect.selectedIndex].getAttribute('lang')), 'GET', function (d) {
            ...
            if (translatedContent) {
                translatedContent.textContent = d.res;
            }
            // Expand box when translation is written
            if (typeof expandDisplayArea === 'function') expandDisplayArea(true);
        ...
        result: function (e, r) {
            ...
            for (var i = e.resultIndex; i < e.results.length; ++i) {
                if (e.results[i].isFinal) {
                    // Display spoken text for final result
                    if (spokenContent) {
                        spokenContent.textContent = e.results[i][0].transcript;
                    }
                    eParrot.prototype.gTranslator(e.results[i][0].transcript);
                } else {
                    // Display spoken text for interim result
                    if (spokenContent) {
                        spokenContent.textContent = e.results[i][0].transcript;
                    }
                    eParrot.prototype.gTranslator(e.results[i][0].transcript);
            ...
            // Clear spoken and translated text areas
            if (spokenContent) spokenContent.textContent = "";
            if (translatedContent) translatedContent.textContent = "";
            // Collapse box when stopped
            if (typeof expandDisplayArea === 'function') expandDisplayArea(false);
        },
        begin: function (r) {
